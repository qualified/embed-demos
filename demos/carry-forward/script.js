...
`,
  ...
  const getSolnHandler = (e) =>
    context.editor.setFileContents({
      code: solutions[editorConfig.challengeId],
    });
  ...
  const nextChallenge = (editor, challengeId) => {
    ...
    nextChallengeBtnHandler = (e) => {
      nextChallengeBtn.disabled = true;
      const nextIdx =
        (1 + challengeIds.indexOf(challengeId)) % challengeIds.length;
      editor.update({ challengeId: challengeIds[nextIdx] });
      editorConfig.challengeId = challengeIds[nextIdx];
      initialFiles.code =
        candidateCode + (presetCodeForChallenge[challengeIds[nextIdx]] || "");
